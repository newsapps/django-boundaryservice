...
class Command(BaseCommand):
    ...
    Fails if the file already exists. Requires that the SHAPEFILES_DIR setting
    is configured.

    You can force the creation of a new file by adding the `-f` or `--force``
    flag.
    ...
    custom_options = (
        make_option(
            ...
            help='Force the creation of a new definitions.py, even if it '
                 'already exists.'
        ),
    ...
    def handle(self, *args, **options):
        if not os.path.exists(options['data_dir']):
            raise CommandError('The shapefiles directory does not exist. '
                               'Create it or specify a different directory.')
...
SHAPEFILES = {
    ...
    'City Council Districts': {
        ...
        # Should the singular name come first when creating canonical
        # identifiers for this set?
        'kind_first': False,
        # Function which each feature wall be passed to in order to extract its
        # "external_id" property
        # The utils module contains several generic functions for doing this
        'ider': utils.simple_namer(['DISTRICT']),
        # Function which each feature will be passed to in order to extract its
        # "name" property
        ...
        # Notes identifying any pecularities about the data, such as columns
        # that were deleted or files which were merged
        'notes': '',
        # Encoding of the text fields in the shapefile, i.e. 'utf-8'. If this
        # is left empty 'ascii' is assumed
        'encoding': '',
        # SRID of the geometry data in the shapefile if it can not be inferred
        # from an accompanying .prj file
        # This is normally not necessary and can be left undefined or set to an
        # empty string to maintain the default behavior
        'srid': '',
